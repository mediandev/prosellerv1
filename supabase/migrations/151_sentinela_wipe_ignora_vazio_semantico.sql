-- Migration 151: sentinela — "vazio escrito de outro jeito" não é perda de dado.
--
-- A regra `wipe_campo_cliente` (migration 150) compara o valor anterior com string
-- vazia. Mas alguns campos guardam vazio em outro formato:
--   * `vendedoresatribuidos` é um array JSON — vazio é `[]`, que como TEXTO não é ''.
--   * `nome_fantasia` tinha `--` num cadastro (placeholder, não conteúdo).
-- Resultado: 6 dos 22 alertas do levantamento de 2026-08-01 eram falso positivo
-- (5 × `[]` em vendedoresatribuidos + 1 × `--` em nome_fantasia).
--
-- Correção: uma função de "conteúdo real" usada na detecção. Um verificador que
-- grita por vazio→vazio treina o time a ignorar o alarme, que é o defeito que a
-- sentinela existe para evitar.
--
-- Só altera a Regra 5; as outras 6 ficam idênticas à 150.

CREATE OR REPLACE FUNCTION public.sentinela_tem_conteudo(p_valor text)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
AS $function$
  SELECT COALESCE(NULLIF(btrim(COALESCE(p_valor, '')), ''), '') <> ''
     AND btrim(COALESCE(p_valor, '')) NOT IN ('[]', '{}', 'null', 'NULL', '--', '-', 'undefined');
$function$;

COMMENT ON FUNCTION public.sentinela_tem_conteudo(text) IS
  'true quando o texto representa conteúdo real (não vazio, não placeholder, não coleção vazia).';

-- Regra 5 revisada; as demais idênticas à migration 150.
