-- Migration 153: sentinela passa a vigiar as falhas de consulta do Simples Nacional.
--
-- Origem: ao investigar se valia construir uma fila com throttle para a consulta
-- à Receita (item 7 do lote de julho), os números mostraram que o risco era
-- teórico. O que existia de real era outra coisa: quando a consulta falha, o
-- registro vai para `regime_lookup_falha` e ninguém olha. A única falha em ~4
-- meses só apareceu porque fomos procurar.
--
-- O usuário JÁ recebe mensagem clara e pode tentar de novo. Esta regra fecha o
-- lado de dentro: a equipe fica sabendo.
--
-- Só acrescenta a Regra 8; as sete anteriores são idênticas à migration 151.
