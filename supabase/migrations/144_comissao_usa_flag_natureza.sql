-- Migration 144: comissão passa a usar o flag `tem_comissao` da natureza de operação.
-- ANTES: comparava o TEXTO `natureza_operacao = 'Bonificação'` — sensível a acento/
-- maiúscula e ignorava outras naturezas marcadas como sem comissão.
-- DEPOIS: consulta natureza_operacao.tem_comissao pelo natureza_id do pedido.
-- Fallback permissivo (natureza ausente => gera) preserva o comportamento atual.
-- Impacto medido em prod: 2 pedidos mudam de critério (9200 e 9221, 'Remessa para
-- troca'), ambos SEM comissão gerada hoje => nenhuma comissão existente é afetada.
-- Decisão do cliente (2026-07-28): usar as opções 'gera receita?'/'gera comissão?'
-- do cadastro de naturezas.
...
AS $function$
  ...
  begin
    ...
    -- 1.1) Regra: a natureza de operação define se gera comissão.
    -- Usa o flag `tem_comissao` do cadastro (fonte da verdade), em vez de comparar
    -- o texto 'Bonificação' (que era sensível a acento/maiúscula e ignorava as
    -- demais naturezas sem comissão, ex.: "Remessa para troca").
    -- Fallback permissivo: natureza ausente/não cadastrada => gera (comportamento anterior).
    select no.tem_comissao
      into v_tem_comissao
      from public.natureza_operacao no
     where no.id = v_pedido.natureza_id;

    if coalesce(v_tem_comissao, true) = false then
      return query select null::bigint, 'natureza_sem_comissao';
      return;
    end if;
