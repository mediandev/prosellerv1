-- Migration 150: SENTINELA — generaliza a regra de "campo apagado".
--
-- Contexto: a regra 5 da migration 148 (`wipe_observacao`) vigiava UM único campo
-- (`cliente_contato.observacao`). O bug da Época (migration 140) apagou 121 campos
-- em 94 clientes espalhados por `cliente`, `cliente_contato` e `cliente_endereço`
-- — a regra antiga só teria visto uma fração.
--
-- Desenho medido em prod (2026-08-01), não teórico:
--   * "campo apagado" cru = 520 falsos positivos só em `codigo` (o histórico registra
--     o apagamento mas o valor está presente hoje: 0 clientes sem código).
--   * "campo apagado E AINDA vazio na base hoje" = 4 casos reais em 30 dias,
--     todos `cliente_contato.observacao` — exatamente a classe do bug da Época.
-- Por isso a regra confirma contra o estado ATUAL da base antes de alertar.
--
-- A regra antiga `wipe_observacao` fica coberta por esta (superconjunto) e para de
-- ser alimentada. Alertas antigos dela não são tocados (havia 0 abertos em 2026-08-01).

CREATE OR REPLACE FUNCTION public.sentinela_campo_ainda_vazio(
  p_tabela  text,
  p_cliente bigint,
  p_coluna  text
) RETURNS boolean
...
DECLARE
  v_existe boolean;
BEGIN
  -- Guarda 1: coluna inexistente (campo renomeado/legado no histórico) geraria
  -- alerta eterno, porque to_jsonb(t)->>'coluna_que_nao_existe' é sempre NULL.
  ...
  -- true = nenhuma linha do cliente tem valor nessa coluna (perda ainda vigente).
  -- Tabelas fixas (allowlist) — nunca interpola entrada externa.
  IF p_tabela = 'cliente' THEN
    -- Guarda 2: cliente excluído (soft delete) não é perda de dado relevante.
    IF NOT EXISTS (SELECT 1 FROM cliente c
                    WHERE c.cliente_id = p_cliente AND c.deleted_at IS NULL) THEN
      RETURN false;
    END IF;
    SELECT EXISTS (SELECT 1 FROM cliente t
                    WHERE t.cliente_id = p_cliente
                      AND COALESCE(to_jsonb(t) ->> p_coluna, '') <> '')
      INTO v_existe;
  ELSIF p_tabela IN ('cliente_contato', 'cliente_endereço')
        AND NOT EXISTS (SELECT 1 FROM cliente c
                         WHERE c.cliente_id = p_cliente AND c.deleted_at IS NULL) THEN
    RETURN false;
  ELSIF p_tabela = 'cliente_contato' THEN
    SELECT EXISTS (SELECT 1 FROM cliente_contato t
                    WHERE t.cliente_id = p_cliente
                      AND COALESCE(to_jsonb(t) ->> p_coluna, '') <> '')
      INTO v_existe;
  ELSIF p_tabela = 'cliente_endereço' THEN
    SELECT EXISTS (SELECT 1 FROM "cliente_endereço" t
                    WHERE t.cliente_id = p_cliente
                      AND COALESCE(to_jsonb(t) ->> p_coluna, '') <> '')
      INTO v_existe;
  ELSE
    RETURN false;  -- tabela fora da allowlist: não alerta
  END IF;
