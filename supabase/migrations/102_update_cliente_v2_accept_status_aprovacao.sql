-- Add support for status_aprovacao update in update_cliente_v2.
-- Keeps backward compatibility with current callers and allows passing
-- p_status_aprovacao from edge function payload.
...
BEGIN
  PERFORM public.update_cliente_v2(
    ...
    p_nome := p_nome,
    p_nome_fantasia := p_nome_fantasia,
    p_cpf_cnpj := p_cpf_cnpj,
    p_ref_tipo_pessoa_id_fk := p_ref_tipo_pessoa_id_fk,
    p_inscricao_estadual := p_inscricao_estadual,
    p_codigo := p_codigo,
    p_grupo_rede := p_grupo_rede,
    p_grupo_id := p_grupo_id,
    p_lista_de_preco := p_lista_de_preco,
    p_desconto_financeiro := p_desconto_financeiro,
    p_pedido_minimo := p_pedido_minimo,
    p_vendedoresatribuidos := p_vendedoresatribuidos,
    p_observacao_interna := p_observacao_interna,
    p_segmento_id := p_segmento_id,
    p_ref_situacao_id := p_ref_situacao_id,
    p_telefone := p_telefone,
    p_telefone_adicional := p_telefone_adicional,
    p_email := p_email,
    p_email_nf := p_email_nf,
    p_website := p_website,
    p_observacao_contato := p_observacao_contato,
    p_cep := p_cep,
    p_rua := p_rua,
    p_numero := p_numero,
    ...
    p_cidade := p_cidade,
    p_uf := p_uf,
    p_ref_tipo_endereco_id_fk := p_ref_tipo_endereco_id_fk,
    p_empresa_faturamento_id := p_empresa_faturamento_id,
    p_condicoes_pagamento_ids := p_condicoes_pagamento_ids,
    p_atualizado_por := p_atualizado_por
  );
