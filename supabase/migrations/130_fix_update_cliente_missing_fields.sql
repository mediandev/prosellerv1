-- Migration 115: Add missing fields to update_cliente_v2
-- Fields that the UI collects but the RPC never persisted on update:
--   desconto (desconto padrão/comercial)
--   condicao_padrao
-- Root cause of "save field A erases field B" reported by client.

-- Drop both overloads that may exist in production
-- 36-param version (from migration 105, without p_desconto)
...
-- 37-param version (older, with p_desconto at position 11)
DROP FUNCTION IF EXISTS public.update_cliente_v2(
  bigint, text, text, text, bigint, text, text, text, uuid, bigint, numeric, numeric, numeric, uuid[],
  text, bigint, integer, text, text, text, text, text, text, text, text, text, text, text,
  text, text, bigint, bigint, bigint[], uuid, text, boolean, jsonb
);
...
CREATE FUNCTION public.update_cliente_v2(
  ...
  -- New params (migration 115)
  p_desconto numeric DEFAULT NULL::numeric,
  p_condicao_padrao bigint DEFAULT NULL::bigint
)
...
GRANT EXECUTE ON FUNCTION public.update_cliente_v2 TO authenticated;
