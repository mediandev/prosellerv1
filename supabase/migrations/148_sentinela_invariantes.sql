-- Migration 148: SENTINELA de invariantes — detecção de violação antes do cliente reclamar.
-- Roda diariamente (pg_cron) verificando 7 regras do contrato do sistema; violações
-- entram em `sentinela_alerta` (dedup por chave) e são AUTO-RESOLVIDAS quando somem.
-- Alerta "aberto" (resolvido_em IS NULL) = violação ativa AGORA.
-- Baseline medido em 2026-07-31: 7/7 regras com ZERO violações.

CREATE TABLE IF NOT EXISTS public.sentinela_alerta (
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_sentinela_alerta_chave_aberta
  ON public.sentinela_alerta (chave) WHERE resolvido_em IS NULL;

ALTER TABLE public.sentinela_alerta ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "authenticated_read_sentinela" ON public.sentinela_alerta;
CREATE POLICY "authenticated_read_sentinela" ON public.sentinela_alerta
...
BEGIN
  ...
  -- ===== Regra 5: observação de contato apagada nas últimas 24h (invariante migration 140) =====
  INSERT INTO sentinela_alerta (regra, chave, detalhe)
  SELECT DISTINCT 'wipe_observacao',
         'wipe_observacao:' || h.cliente_id || ':' || h.data_hora::date,
         jsonb_build_object('cliente_id', h.cliente_id, 'quando', h.data_hora)
    FROM cliente_historico_alteracoes h, jsonb_array_elements(h.campos_alterados) e
   WHERE h.data_hora > now() - interval '24 hours'
     AND e->>'campo' = 'cliente_contato.observacao'
     AND e->'valorNovo' = 'null'::jsonb AND e->'valorAnterior' <> 'null'::jsonb
  ON CONFLICT (chave) WHERE resolvido_em IS NULL DO NOTHING;
  GET DIAGNOSTICS v_cnt = ROW_COUNT; v_novos := v_novos + v_cnt;
  -- (sem auto-resolução: é um evento pontual; resolver manualmente após restaurar)
...
-- Agenda diária (6h BRT = 9h UTC). pg_cron já habilitado (migration 139).
SELECT cron.unschedule('sentinela-diaria')
 WHERE EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'sentinela-diaria');

SELECT cron.schedule('sentinela-diaria', '0 9 * * *', 'SELECT public.sentinela_verificar();');
