-- Migration 140: corrige UPSERT de cliente_contato e cliente_endereço na RPC update_cliente_v2.
-- BUG: o fallback do COALESCE usava EXCLUDED.<col> (a linha PROPOSTA para inserção,
-- = o próprio parâmetro null) em vez do valor EXISTENTE na tabela. Resultado: ao salvar
-- um cliente sem reenviar um campo (ex.: observacao de contato, que o form nem carrega),
-- o campo era APAGADO. Afetou ~89 clientes (observacao) desde 2026-03-20.
-- FIX: trocar EXCLUDED.<col> -> cliente_contato.<col> / "cliente_endereço".<col> (valor atual).
-- Recuperação dos dados perdidos: ver migration/script de backfill a partir de
-- cliente_historico_alteracoes.valorAnterior.
