-- ============================================================================
-- Migration 069: Fix list_conta_corrente_v2 DISTINCT ORDER BY error
-- ============================================================================
-- Descrição: Corrige erro "for SELECT DISTINCT, ORDER BY expressions must appear in select list"
--            na função list_conta_corrente_v2 usando DISTINCT ON
-- Data: 2026-02-04
-- ============================================================================

-- Corrigir a função list_conta_corrente_v2
...
BEGIN
  ...
  SELECT COALESCE(JSON_AGG(
    JSON_BUILD_OBJECT(
      ...
      'clienteId', compromisso_completo.cliente_id::TEXT,
      'clienteNome', compromisso_completo.cliente_nome,
      'clienteGrupoRede', compromisso_completo.cliente_grupo_rede,
      'vendedorUuid', compromisso_completo.vendedor_uuid::TEXT,
      'data', compromisso_completo.data,
      'valor', compromisso_completo.valor,
      'titulo', compromisso_completo.titulo,
      'descricao', compromisso_completo.descricao_longa,
      'tipoCompromisso', INITCAP(compromisso_completo.tipo_compromisso),
      ...
      'status', compromisso_completo.status,
...
'Lista compromissos de conta corrente com filtros, paginação e estatísticas (corrigido DISTINCT ORDER BY)';
