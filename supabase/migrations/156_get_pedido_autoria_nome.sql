-- Migration 156: o detalhe do pedido passa a mostrar QUEM criou e QUEM alterou.
--
-- A migration 155 fez o banco guardar a autoria. Esta expõe o NOME na consulta do
-- pedido — sem isso a tela receberia só o identificador interno (createdBy), que
-- não diz nada a ninguém.
--
-- Dois LEFT JOIN em `user`: se o usuário não existir mais (ou o pedido for antigo,
-- sem autoria), o nome vem nulo e a tela simplesmente não mostra a linha. Nenhum
-- campo existente foi alterado ou removido — só três acrescentados, então nada
-- que já consumia essa consulta quebra.
...
BEGIN
  ...
  SELECT JSON_BUILD_OBJECT(
    ...
    'criadoPorNome', uc.nome,
    'atualizadoPorNome', uu.nome,
    'atualizadoEm', pv.updated_at,
    'idTiny', pv.id_tiny,
  ...
  LEFT JOIN public."user" uc ON uc.user_id = pv.created_by
  LEFT JOIN public."user" uu ON uu.user_id = pv.updated_by
