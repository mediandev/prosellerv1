-- Migration 146: CEP guardado só com dígitos (decisão do cliente 2026-07-31).
-- ANTES: normalização removia apenas o hífen (REPLACE '-'), deixando o ponto —
-- origem dos CEPs corrompidos tipo '13.345400'. DEPOIS: regexp_replace remove
-- todo não-dígito. Coluna é TEXT ⇒ zeros à esquerda preservados (498 CEPs
-- começam com 0). Exibição com máscara (NN.NNN-NNN) fica no front.
-- Compatível com a emissão: tiny-enviar já aplica digitsOnly(cep) ao enviar.
...
BEGIN
  ...
  IF p_cep IS NOT NULL OR p_rua IS NOT NULL OR p_numero IS NOT NULL OR p_complemento IS NOT NULL OR p_bairro IS NOT NULL OR p_cidade IS NOT NULL OR p_uf IS NOT NULL THEN
    ...
    VALUES (
      v_cliente_id,
      NULLIF(regexp_replace(TRIM(COALESCE(p_cep, '')), '\D', '', 'g'), ''),
      NULLIF(TRIM(p_rua), ''),
    ...
    ON CONFLICT (cliente_id) DO UPDATE SET
      cep = COALESCE(NULLIF(regexp_replace(TRIM(COALESCE(p_cep, '')), '\D', '', 'g'), ''), "cliente_endereço".cep),
