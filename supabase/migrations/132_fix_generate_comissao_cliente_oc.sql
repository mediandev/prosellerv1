-- 132_fix_generate_comissao_cliente_oc.sql
-- Bug: relatório de comissões com colunas "OC Cliente" e "Cliente" em branco.
-- Causa: generate_vendedor_comissao parou de gravar oc_cliente / cliente_id / cliente_nome
--        nas linhas de vendedor_comissão (INSERT e UPDATE só preenchiam valores).
--        Quebrou a partir de abr/2026 (jan-mar tinham os campos preenchidos).
-- Fix: carregar ordem_cliente e nome_cliente do pedido e gravá-los em ambos os ramos.
--      NÃO altera nenhum valor de comissão (valor_total, valor_comissao, percentual).
-- Fonte (idêntica ao histórico jan-mar): pedido_venda.ordem_cliente, .nome_cliente, .cliente_id.
