  ...
  try {
    ...
    if (req.method === 'GET') {
      if (id) {
        ...
        if (error || !data) return jsonResponse(404, { success: false, error: 'Origem não encontrada' })
      ...
      let query = supabase.from('origem_frete').select('*', { count: 'exact' }).order('nome', { ascending: true })
      if (empresaIdFilter) query = query.eq('empresa_id', empresaIdFilter)
      if (apenasAtivos) query = query.eq('ativo', true)
      const { data, error, count } = await query
      if (error) throw new Error(error.message)
      return jsonResponse(200, { success: true, data: { origens: (data || []).map(format), total: count ?? 0 } })
    }

    if (req.method !== 'GET' && user.tipo !== 'backoffice') {
      return jsonResponse(403, { success: false, error: 'Apenas backoffice pode escrever origens' })
    }
    ...
    if (req.method === 'POST') {
      ...
      if (error) {
        if ((error as { code?: string }).code === '23505') return jsonResponse(409, { success: false, error: 'Origem já cadastrada para esta empresa' })
    ...
    if (req.method === 'DELETE') {
      if (!id) return jsonResponse(400, { success: false, error: 'ID obrigatório' })
      const { error } = await supabase.from('origem_frete').update({ ativo: false }).eq('id', id)
      if (error) throw new Error(error.message)
      return jsonResponse(200, { success: true, data: { message: 'Origem desativada' } })
    }
