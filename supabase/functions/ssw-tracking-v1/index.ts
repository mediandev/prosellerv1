...
async function validateJWT(req: Request, supabaseUrl: string, supabaseServiceKey: string) {
  ...
  if (!authHeader) return { userId: null as string | null, error: 'Missing authorization header' }
  const token = authHeader.replace('Bearer ', '')
  const supabase = createClient(supabaseUrl, supabaseServiceKey, {
    global: { headers: { Authorization: authHeader } },
  })
  const { data: { user }, error } = await supabase.auth.getUser(token)
  if (error || !user) return { userId: null as string | null, error: 'Invalid or expired token' }
  const { data: userData, error: userError } = await supabase
    .from('user').select('user_id, ativo')
    .eq('user_id', user.id).eq('ativo', true).is('deleted_at', null).single()
  if (userError || !userData) return { userId: null as string | null, error: 'User not found or inactive' }
  return { userId: userData.user_id as string, error: null as string | null }
}
...
serve(async (req) => {
  ...
  if (!isLogCrmSswEnabledFromEnv()) {
    return jsonResponse(503, { success: false, error: 'SSW Tracking desligado (FEATURE_LOG_CRM_SSW)' })
  }
  ...
  try {
    ...
    if (authError || !userId) return jsonResponse(401, { success: false, error: authError || 'Unauthorized' })

    const url = new URL(req.url)
    const chaveNfe = url.searchParams.get('chave_nfe')

    if (!chaveNfe || !/^[0-9]{44}$/.test(chaveNfe)) {
      return jsonResponse(400, { success: false, error: 'chave_nfe deve ter 44 dígitos numéricos' })
    }
