...
serve(async (req) => {
  ...
  try {
    ...
    const { data: pedido, error: pedidoError } = await supabase
      .from('pedido_venda')
      .select('pedido_venda_ID,id_tiny,empresa_faturamento_id,tiny_natureza_enviada,tiny_optante_aplicado,tiny_fallback_used,numero_pedido,cliente_id')
      .eq('pedido_venda_ID', pedidoId)
      .maybeSingle()

    if (pedidoError) throw createBadRequestError('Falha ao buscar pedido', { code: pedidoError.code, message: pedidoError.message })
    if (!pedido) throw createNotFoundError('Pedido', String(pedidoId))
    if (!pedido.id_tiny) throw createBadRequestError('Pedido nao tem id_tiny — nao foi enviado ao Tiny ainda', { pedido_venda_ID: pedidoId })

    const { data: empresa, error: empresaError } = await supabase
      .from('ref_empresas_subsidiarias')
      .select('id, nome, chave_api')
      .eq('id', pedido.empresa_faturamento_id)
      .is('deleted_at', null)
      .maybeSingle()

    if (empresaError) throw createBadRequestError('Falha ao buscar empresa', { code: empresaError.code, message: empresaError.message })
    if (!empresa?.chave_api) throw createBadRequestError('Empresa sem chave_api configurada', { empresa_id: pedido.empresa_faturamento_id })
    ...
    let tinyData: any = null
    try { tinyData = JSON.parse(tinyText) } catch { /* tiny pode retornar erro em texto */ }
    ...
    return createHttpSuccessResponse(
      {
        ...
        bate: pedido.tiny_natureza_enviada
          ? (String(pedido.tiny_natureza_enviada) === String(idNaturezaNoTiny || naturezaNoTiny))
          : null,
        tiny_raw: tinyData,
      },
      200,
