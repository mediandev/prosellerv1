...
function formatTransportador(row) {
  const cnpj = row.cnpj || '';
  return {
    id: String(row.id),
...
serve(async (req)=>{
  ...
  try {
    ...
    if (req.method === 'GET') {
      if (id) {
        const { data, error } = await supabase.from('transportador_logistica').select('*').eq('id', id).is('deleted_at', null).single();
        ...
        return jsonResponse(200, {
          success: true,
          data: formatTransportador(data)
        });
      }
      const apenasAtivos = url.searchParams.get('apenas_ativos') === 'true';
      let query = supabase.from('transportador_logistica').select('*', {
        count: 'exact'
      ...
      return jsonResponse(200, {
        success: true,
        data: {
          transportadores: (data || []).map(formatTransportador),
          total: count ?? 0
        }
      });
    }
    ...
    if (req.method !== 'GET' && user.tipo !== 'backoffice') {
      return jsonResponse(403, {
        success: false,
        error: 'Apenas backoffice pode escrever transportadores'
      });
    }
    // ---------- POST ----------
    if (req.method === 'POST') {
      ...
      if (cnpjDigits.length !== 14) return jsonResponse(400, {
        success: false,
        error: 'CNPJ inválido (esperado 14 dígitos)'
      });
      if (!body.razaoSocial || String(body.razaoSocial).trim().length < 2) {
        return jsonResponse(400, {
          success: false,
          error: 'razaoSocial obrigatório'
        });
      }
      const insert = {
      ...
      const { data, error } = await supabase.from('transportador_logistica').insert(insert).select().single();
      if (error) {
        if (error.code === '23505') {
      ...
      console.log(`[${FUNC_NAME}] created`, {
        id: data.id,
        cnpjMasked: maskCnpj(cnpjDigits)
      });
    ...
    if (req.method === 'PUT') {
      ...
      if (body.cnpj !== undefined) {
        ...
        if (c.length !== 14) return jsonResponse(400, {
          success: false,
          error: 'CNPJ inválido'
        });
        patch.cnpj = c;
      ...
      const { data, error } = await supabase.from('transportador_logistica').update(patch).eq('id', id).is('deleted_at', null).select().single();
      if (error || !data) return jsonResponse(404, {
        success: false,
        error: 'Transportador não encontrado para atualizar'
      });
      return jsonResponse(200, {
        success: true,
        data: formatTransportador(data)
    ...
    if (req.method === 'DELETE') {
      ...
      const { error } = await supabase.from('transportador_logistica').update({
      ...
      return jsonResponse(200, {
        success: true,
        data: {
          message: 'Transportador removido'
        }
      });
    }
