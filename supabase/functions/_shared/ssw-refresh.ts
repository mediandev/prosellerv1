...
export async function sweepSsw(
  ...
  // Processa em lotes concorrentes para não estourar o tempo da edge.
  // SSW aceita até 20 req/s — concorrência de 8 fica folgada.
  const CONCURRENCIA = 8
  for (let i = 0; i < candidatos.length; i += CONCURRENCIA) {
    const lote = candidatos.slice(i, i + CONCURRENCIA)
    const resultados = await Promise.all(
      lote.map((frete) =>
        refreshSswForFrete(supabase, frete, { force: opts.force })
          .catch(() => ({ polled: false } as RefreshResult)),
      ),
    )
    atualizados += resultados.filter((r) => r.polled).length
  }
