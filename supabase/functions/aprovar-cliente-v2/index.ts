...
serve(async (req) => {
  ...
  console.log('[APROVAR-CLIENTE-V2] Request received:', { method: req.method, url: req.url, timestamp: new Date().toISOString() })
  ...
  try {
    console.log('[APROVAR-CLIENTE-V2] Step 1: Starting authentication...')
    const supabaseUrl = Deno.env.get('SUPABASE_URL')!
    const supabaseServiceKey = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!
    const { user, error: authError } = await validateJWT(req, supabaseUrl, supabaseServiceKey)

    if (authError || !user) {
      console.error('[APROVAR-CLIENTE-V2] Authentication failed:', { error: authError })
      return createAuthErrorResponse(authError || 'Unauthorized')
    }

    console.log('[APROVAR-CLIENTE-V2] Authentication successful:', { userId: user.id, tipo: user.tipo })

    console.log('[APROVAR-CLIENTE-V2] Step 2: Checking permissions...')
    if (user.tipo !== 'backoffice') {
      console.error('[APROVAR-CLIENTE-V2] Permission denied:', { userTipo: user.tipo })
      return createPermissionErrorResponse('Apenas usuários backoffice podem aprovar clientes')
    }

    console.log('[APROVAR-CLIENTE-V2] Step 3: Extracting cliente_id from URL...')
    const url = new URL(req.url)
    const pathParts = url.pathname.split('/')
    const clienteId = pathParts[pathParts.length - 1]
    console.log('[APROVAR-CLIENTE-V2] Extracted clienteId:', clienteId)

    if (!clienteId || clienteId === 'aprovar-cliente-v2') {
      console.error('[APROVAR-CLIENTE-V2] ERROR: cliente_id is missing or invalid')
      throw new ValidationError('cliente_id é obrigatório na URL')
    }

    const clienteIdNum = parseInt(clienteId)
    if (isNaN(clienteIdNum)) {
      console.error('[APROVAR-CLIENTE-V2] ERROR: cliente_id is not a valid number')
      throw new ValidationError('cliente_id deve ser um número válido')
    }

    console.log('[APROVAR-CLIENTE-V2] Step 4: Calling RPC function...', { p_cliente_id: clienteIdNum, p_aprovado_por: user.id })
    const supabase = createClient(supabaseUrl, supabaseServiceKey, {
      global: { headers: { Authorization: req.headers.get('Authorization') || '' } },
    })

    const { data: rpcData, error: rpcError } = await supabase.rpc('aprovar_cliente_v2', {
      p_cliente_id: clienteIdNum,
      p_aprovado_por: user.id,
    })

    if (rpcError) {
      console.error('[APROVAR-CLIENTE-V2] RPC Error:', { message: rpcError.message, details: rpcError.details, code: rpcError.code })
      throw new Error(`Database operation failed: ${rpcError.message}`)
    }

    console.log('[APROVAR-CLIENTE-V2] RPC call successful:', { dataLength: rpcData?.length || 0 })

    const duration = Date.now() - startTime
    console.log(`[APROVAR-CLIENTE-V2] SUCCESS: Cliente approved: ${clienteId} by ${user.id} (${duration}ms)`)

    return createHttpSuccessResponse(
      {
        cliente_id: rpcData[0]?.cliente_id,
        status_aprovacao: rpcData[0]?.status_aprovacao,
        data_aprovacao: rpcData[0]?.data_aprovacao,
    ...
    const duration = Date.now() - startTime
    console.error('[APROVAR-CLIENTE-V2] EXCEPTION:', {
