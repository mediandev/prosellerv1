...
serve(async (req) => {
  ...
  console.log('[DELETE-USER-V2] Request received:', { method: req.method, url: req.url, timestamp: new Date().toISOString() })
  ...
  try {
    console.log('[DELETE-USER-V2] Step 1: Starting authentication...')
    const supabaseUrl = Deno.env.get('SUPABASE_URL')!
    const supabaseServiceKey = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!
    const { user, error: authError } = await validateJWT(req, supabaseUrl, supabaseServiceKey)

    if (authError || !user) {
      console.error('[DELETE-USER-V2] Authentication failed:', { error: authError })
      return createAuthErrorResponse(authError || 'Unauthorized')
    }

    console.log('[DELETE-USER-V2] Authentication successful:', { userId: user.id, tipo: user.tipo })

    console.log('[DELETE-USER-V2] Step 2: Checking permissions...')
    if (user.tipo !== 'backoffice') {
      console.error('[DELETE-USER-V2] Permission denied:', { userTipo: user.tipo })
      return createPermissionErrorResponse('Apenas usuários backoffice podem excluir usuários')
    }

    console.log('[DELETE-USER-V2] Step 3: Extracting user_id from URL...')
    const url = new URL(req.url)
    const pathParts = url.pathname.split('/')
    const userId = pathParts[pathParts.length - 1]
    console.log('[DELETE-USER-V2] Extracted userId:', userId)

    if (!userId || userId === 'delete-user-v2') {
      console.error('[DELETE-USER-V2] ERROR: user_id is missing or invalid')
      throw new ValidationError('user_id é obrigatório na URL')
    }

    console.log('[DELETE-USER-V2] Step 4: Calling RPC function...', { p_user_id: userId, p_deleted_by: user.id })
    const supabase = createClient(supabaseUrl, supabaseServiceKey, {
      global: { headers: { Authorization: req.headers.get('Authorization') || '' } },
    })

    const { data: rpcData, error: rpcError } = await supabase.rpc('delete_user_v2', {
      p_user_id: userId,
      p_deleted_by: user.id,
    })

    if (rpcError) {
      console.error('[DELETE-USER-V2] RPC Error:', { message: rpcError.message, details: rpcError.details, code: rpcError.code })
      throw new Error(`Database operation failed: ${rpcError.message}`)
    }

    console.log('[DELETE-USER-V2] RPC call successful:', { dataLength: rpcData?.length || 0 })

    const duration = Date.now() - startTime
    console.log(`[DELETE-USER-V2] SUCCESS: User deleted: ${userId} by ${user.id} (${duration}ms)`)

    return createHttpSuccessResponse(
      { user_id: rpcData[0]?.user_id, deleted_at: rpcData[0]?.deleted_at, message: 'Usuário excluído com sucesso' },
      200,
      { userId: user.id, duration: `${duration}ms` }
    )
  } catch (error) {
    const duration = Date.now() - startTime
    console.error('[DELETE-USER-V2] EXCEPTION:', {
