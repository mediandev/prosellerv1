...
serve(async (req) => {
  ...
  console.log('[DELETE-CLIENTE-V2] Request received:', { method: req.method, url: req.url, timestamp: new Date().toISOString() })
  ...
  try {
    console.log('[DELETE-CLIENTE-V2] Step 1: Starting authentication...')
    const supabaseUrl = Deno.env.get('SUPABASE_URL')!
    const supabaseServiceKey = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!
    const { user, error: authError } = await validateJWT(req, supabaseUrl, supabaseServiceKey)

    if (authError || !user) {
      console.error('[DELETE-CLIENTE-V2] Authentication failed:', { error: authError })
      return createAuthErrorResponse(authError || 'Unauthorized')
    }

    console.log('[DELETE-CLIENTE-V2] Authentication successful:', { userId: user.id, tipo: user.tipo })

    console.log('[DELETE-CLIENTE-V2] Step 2: Checking permissions...')
    if (user.tipo !== 'backoffice') {
      console.error('[DELETE-CLIENTE-V2] Permission denied:', { userTipo: user.tipo })
      return createPermissionErrorResponse('Apenas usuários backoffice podem excluir clientes')
    }

    console.log('[DELETE-CLIENTE-V2] Step 3: Extracting cliente_id from URL...')
    const url = new URL(req.url)
    const pathParts = url.pathname.split('/')
    const clienteId = pathParts[pathParts.length - 1]
    console.log('[DELETE-CLIENTE-V2] Extracted clienteId:', clienteId)

    if (!clienteId || clienteId === 'delete-cliente-v2') {
      console.error('[DELETE-CLIENTE-V2] ERROR: cliente_id is missing or invalid')
      throw new ValidationError('cliente_id é obrigatório na URL')
    }

    const clienteIdNum = parseInt(clienteId)
    if (isNaN(clienteIdNum)) {
      console.error('[DELETE-CLIENTE-V2] ERROR: cliente_id is not a valid number')
      throw new ValidationError('cliente_id deve ser um número válido')
    }

    console.log('[DELETE-CLIENTE-V2] Step 4: Calling RPC function...', { p_cliente_id: clienteIdNum, p_deleted_by: user.id })
    const supabase = createClient(supabaseUrl, supabaseServiceKey, {
      global: { headers: { Authorization: req.headers.get('Authorization') || '' } },
    })

    const { data: rpcData, error: rpcError } = await supabase.rpc('delete_cliente_v2', {
      p_cliente_id: clienteIdNum,
      p_deleted_by: user.id,
    })

    if (rpcError) {
      console.error('[DELETE-CLIENTE-V2] RPC Error:', { message: rpcError.message, details: rpcError.details, code: rpcError.code })
      throw new Error(`Database operation failed: ${rpcError.message}`)
    }

    console.log('[DELETE-CLIENTE-V2] RPC call successful:', { dataLength: rpcData?.length || 0 })

    const duration = Date.now() - startTime
    console.log(`[DELETE-CLIENTE-V2] SUCCESS: Cliente deleted: ${clienteId} by ${user.id} (${duration}ms)`)

    return createHttpSuccessResponse(
      { cliente_id: rpcData[0]?.cliente_id, deleted_at: rpcData[0]?.deleted_at, message: 'Cliente excluído com sucesso' },
      200,
      { userId: user.id, duration: `${duration}ms` }
    )
  } catch (error) {
    const duration = Date.now() - startTime
    console.error('[DELETE-CLIENTE-V2] EXCEPTION:', {
