...
serve(async (req)=>{
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      let query = supabase.from('regiao_destino').select('*', {
        count: 'exact'
      }).order('nome', {
        ascending: true
      });
    ...
    if (req.method !== 'GET' && user.tipo !== 'backoffice') {
      return jsonResponse(403, {
        success: false,
        error: 'Apenas backoffice pode escrever regiões'
      });
    }
    if (req.method === 'POST') {
    ...
    if (req.method === 'DELETE') {
      ...
      const { error } = await supabase.from('regiao_destino').update({
        ativo: false
      }).eq('id', id);
      if (error) throw new Error(error.message);
      return jsonResponse(200, {
