...
function formatCategoria(row: any) {
  return {
    id: String(row.id),
    nome: row.nome,
    ...
    dataCriacao: row.created_at ? new Date(row.created_at).toISOString() : new Date().toISOString(),
    criadoPor: 'Sistema',
...
serve(async (req) => {
  ...
  console.log('[CATEGORIAS-CONTA-CORRENTE-V2] Request received:', { method: req.method, url: req.url })
  ...
  try {
    ...
    const functionIndex = pathParts.indexOf('categorias-conta-corrente-v2')
    const categoriaId = functionIndex >= 0 && functionIndex < pathParts.length - 1
    ...
    if (req.method === 'GET') {
      if (categoriaId) {
        const { data: rows, error: rpcError } = await supabase
          .rpc('get_categorias_conta_corrente_v2', { p_id: categoriaId })
        if (rpcError) throw new Error(`Database operation failed: ${rpcError.message}`)
        if (!rows || rows.length === 0) throw new Error('Categoria de conta corrente não encontrada')
        const formatted = formatCategoria(rows[0])
      ...
      const { data: rpcData, error: rpcError } = await supabase
        .rpc('list_categorias_conta_corrente_v2', {
      ...
      const categorias = rpcData?.categorias || []
      const formattedList = categorias.map((c: any) => ({
        id: String(c.id),
        nome: c.nome,
        descricao: c.descricao || undefined,
        cor: c.cor || undefined,
        ativo: c.ativo ?? true,
        dataCriacao: c.createdAt ? new Date(c.createdAt).toISOString() : new Date().toISOString(),
        criadoPor: 'Sistema',
        dataAtualizacao: c.updatedAt ? new Date(c.updatedAt).toISOString() : undefined,
        atualizadoPor: 'Sistema',
      }))
    ...
    if (req.method === 'POST') {
      ...
      if (user.tipo !== 'backoffice') {
        throw new Error('Apenas usuários backoffice podem criar categorias de conta corrente')
      }
      ...
      const { data: rows, error: rpcError } = await supabase
        .rpc('create_categorias_conta_corrente_v2', {
          p_nome: body.nome.trim(),
          p_descricao: body.descricao ? body.descricao.trim() : null,
          p_cor: body.cor ? body.cor.trim() : null,
          p_created_by: user.id,
        })
      if (rpcError) throw new Error(`Database operation failed: ${rpcError.message}`)
      if (!rows || rows.length === 0) throw new Error('Erro ao criar categoria de conta corrente')
    ...
    if (req.method === 'PUT') {
      const body = await req.json()
      const id = categoriaId || body.id
      if (!id) throw new Error('ID é obrigatório para atualização')
      if (user.tipo !== 'backoffice') {
        throw new Error('Apenas usuários backoffice podem atualizar categorias de conta corrente')
      }

      const { data: rows, error: rpcError } = await supabase
        .rpc('update_categorias_conta_corrente_v2', {
          p_id: id,
          p_nome: body.nome ? body.nome.trim() : null,
          p_descricao: body.descricao !== undefined ? (body.descricao ? body.descricao.trim() : null) : null,
          p_cor: body.cor !== undefined ? (body.cor ? body.cor.trim() : null) : null,
    ...
    if (req.method === 'DELETE') {
      const id = categoriaId || url.searchParams.get('id')
      if (!id) throw new Error('ID é obrigatório para exclusão')
      if (user.tipo !== 'backoffice') {
        throw new Error('Apenas usuários backoffice podem excluir categorias de conta corrente')
      }

      const { error: deleteError } = await supabase
        .rpc('delete_categorias_conta_corrente_v2', { p_id: id, p_deleted_by: user.id })
      if (deleteError) throw new Error(`Database operation failed: ${deleteError.message}`)

      return createHttpSuccessResponse(
        { success: true, message: 'Categoria de conta corrente excluída com sucesso' },
        200,
        { userId: user.id }
      )
    }
    ...
    console.error('[CATEGORIAS-CONTA-CORRENTE-V2] EXCEPTION:', error instanceof Error ? error.message : String(error))
