...
serve(async (req) => {
  ...
  console.log('[LIST-USERS-V2] Request received:', { method: req.method, url: req.url, timestamp: new Date().toISOString() })
  ...
  try {
    console.log('[LIST-USERS-V2] Step 1: Starting authentication...')
    const supabaseUrl = Deno.env.get('SUPABASE_URL')!
    const supabaseServiceKey = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!
    const { user, error: authError } = await validateJWT(req, supabaseUrl, supabaseServiceKey)

    if (authError || !user) {
      console.error('[LIST-USERS-V2] Authentication failed:', { error: authError })
      return createAuthErrorResponse(authError || 'Unauthorized')
    }

    console.log('[LIST-USERS-V2] Authentication successful:', { userId: user.id, tipo: user.tipo })

    console.log('[LIST-USERS-V2] Step 2: Extracting query parameters...')
    const url = new URL(req.url)
    const tipoFilter = url.searchParams.get('tipo') || null
    const ativoFilter = url.searchParams.get('ativo')
    const search = url.searchParams.get('search') || null
    const page = parseInt(url.searchParams.get('page') || '1')
    const limit = Math.min(parseInt(url.searchParams.get('limit') || '10'), 100)

    console.log('[LIST-USERS-V2] Query parameters:', { tipoFilter, ativoFilter, search, page, limit })

    if (page < 1) {
      console.error('[LIST-USERS-V2] Validation error: Invalid page')
      throw new Error('Page must be greater than 0')
    }
    if (limit < 1) {
      console.error('[LIST-USERS-V2] Validation error: Invalid limit')
      throw new Error('Limit must be greater than 0')
    }

    console.log('[LIST-USERS-V2] Step 3: Calling RPC function...', {
      p_requesting_user_id: user.id,
      p_tipo_filter: tipoFilter,
      p_ativo_filter: ativoFilter ? ativoFilter === 'true' : null,
      p_search: search,
      p_page: page,
      p_limit: limit
    })
    ...
    const { data: rpcData, error: rpcError } = await supabase.rpc('list_users_v2', {
      p_requesting_user_id: user.id,
      p_tipo_filter: tipoFilter,
      p_ativo_filter: ativoFilter ? ativoFilter === 'true' : null,
      p_search: search,
    ...
    if (rpcError) {
      console.error('[LIST-USERS-V2] RPC Error:', { message: rpcError.message, details: rpcError.details, code: rpcError.code })
      throw new Error(`Database operation failed: ${rpcError.message}`)
    }

    console.log('[LIST-USERS-V2] RPC call successful:', {
    ...
    const duration = Date.now() - startTime
    console.error('[LIST-USERS-V2] EXCEPTION:', {
