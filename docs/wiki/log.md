...
- 2026-08-03 · [FEATURE] · Auditoria de pedidos (migrations 155/156 + tela). Levantamento corrigiu a premissa: `created_by` já existia (434/575 preenchidos) e a Edge já enviava o usuário nos 3 caminhos — as RPCs de alterar/excluir recebiam e descartavam. Gatilho próprio porque exclusão é soft delete. Detalhe do pedido passa a mostrar "Criado por X em ...". V 1.82 · SHA ac40b83
- 2026-08-03 · [FEATURE] · Auditoria de clientes (migration 157). `delete_cliente_v2` também recebia e descartava o autor. Correção rotineira de cadastro fica FORA de propósito (já está no histórico) — caso de teste fixa esse silêncio. Decisão registrada: não alterar `update_cliente_v2` (22 KB, causou o incidente de março) só para gravar `aprovado_por`; `atualizado_por` já identifica quem aprovou. V 1.83 · SHA df17747
- 2026-08-03 · [FEATURE] · Auditoria de usuários e permissões (migration 158). Registro guarda o DELTA (ganhou/perdeu), não as listas. Exigiu 1 linha em `update-user-v2`: a troca de permissões grava direto na tabela, sem passar pela RPC — sem ela o autor sairia como 'Sistema'. Testado em prod pelo caminho real: "Lucas alterou as permissões de Cicero Rocha Costa — perdeu vendas.criar", e devolvido. V 1.84 · SHA 45ba09d
