  ...
  return (
    <div className="flex flex-col h-full bg-card border-r">
      ...
      <div className="p-4 border-t flex-shrink-0 bg-card">
        <SidebarUserInfo
          onOpenProfile={() => onPageChange("perfil")}
          onOpenChangelog={() => onPageChange("changelog")}
        />
      </div>
    </div>
  );
        ...
        <div className="flex items-center justify-center gap-1 text-[11px] text-muted-foreground">
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <button
                type="button"
                aria-label="Ver novidades desta versão"
            ...
            <TooltipContent side="top" className="max-w-xs">
              <p className="font-medium mb-1">Novidades em {ultimaVersao.version}</p>
              <ul className="list-disc pl-4 space-y-0.5 text-left">
                {ultimaVersao.items.map((item, i) => (
                  <li key={i}>{item}</li>
                ))}
              </ul>
              <p className="mt-2 text-[11px] text-muted-foreground italic">
                Clique para ver versões anteriores
              </p>
            </TooltipContent>
          </Tooltip>
        </div>
...
const pageConfig: Record<Page, { title: string; description: string }> = {
  ...
  changelog: {
    title: "Novidades",
    description: "Histórico de versões e melhorias do sistema."
  }
};
