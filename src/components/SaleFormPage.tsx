...
export function SaleFormPage({ vendaId, modo, onVoltar }: SaleFormPageProps) {
      ...
      <div className="flex gap-2">
        ...
        {!isReadOnly && !pedidoBloqueado && (
          <>
            ...
            {(modoAtual === 'criar' || (modoAtual === 'editar' && formData.status === 'Rascunho')) ? (
              // Criar OU editar rascunho: mesmos botões (Salvar como Rascunho + Enviar)
              <>
                <Button
                  variant="outline"
              ...
              // Editar pedido já enviado: apenas Salvar Alterações
              <Button onClick={() => handleSave(false)}>
  ...
  const handleSave = async (salvarComoRascunho: boolean = false) => {
    ...
    const vendaCompleta: Venda = {
      ...
      // Salva sempre como Rascunho; se enviar ao ERP com sucesso, atualiza para 'Em aberto'
      status: 'Rascunho',
    ...
    try {
      ...
      } else if (formData.empresaFaturamentoId) {
        // Enviar ao ERP (tanto na criação quanto na edição de rascunho)
